...
**결정 이유**:
1. 스킬 수가 적절 (38개, 단 KB 검색 스킬은 동일 패턴의 경량 래퍼)
2. 개인용 도구 모음
3. 관리 단순화
...
| 인프라/도메인 | opensearch-client, opensearch-server, langgraph, python, unix, openclaw, claude-code, codex, rust, deepagents, hermes-agent, cloudflare-one, langfuse, k3s, kubeflow, argocd, terraform, opentelemetry, github, agentic-engineering, clean-code, obsidian, mlx, mlx-swift, mlx-c | `disable-model-invocation: true` |
...
- [ ] 특정 스킬만 오픈소스로 공개
- [x] ~~스킬이 20개 이상으로 증가~~ → 38개 도달. 단, KB 스킬은 경량 래퍼라 실질 복잡도 낮음
- [ ] 스킬별 버전 관리 필요
